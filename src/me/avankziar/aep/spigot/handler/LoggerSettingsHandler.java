...
import me.avankziar.aep.general.objects.LoggerSettings.OrderType;
...
public class LoggerSettingsHandler
{
	...
	public LoggerSettingsHandler(AEP plugin)
	{
		this.plugin = plugin;
	}
	...
	public void generateGUI(Player player, UUID uuid, int accountID, Inventory openInventory, int page)
	{
		...
		{
			inventory = Bukkit.createInventory(null, 9*6, ChatApiOld.tl("&cLogger &aSettings"));
		}
	...
	public void generateActionFromClick(InventoryClickEvent event) throws IOException
	{
		...
		case 36: //PreSet 1
			...
			{
				if(!plugin.getMysqlHandler().exist(MysqlType.LOGGERSETTINGSPRESET,
						"`slotid` = ? AND `player_uuid` = ?", slotid, uuid.toString()))
				{
					return;
				}
				plugin.getMysqlHandler().deleteData(MysqlType.LOGGERSETTINGSPRESET, "`slotid` = ? AND `player_uuid` = ?", slotid, uuid.toString());
			} else if(event.isLeftClick())
			{
				LoggerSettings ls = (LoggerSettings) plugin.getMysqlHandler().getData(MysqlType.LOGGERSETTINGSPRESET,
						"`slotid` = ? AND `player_uuid` = ?", slotid, uuid.toString());
				if(ls == null)
				{
					return;
			...
			} else if(event.isRightClick())
			{
				...
				plugin.getMysqlHandler().create(MysqlType.LOGGERSETTINGSPRESET, fst);
			}
	...
	public void anvilStringEditorOutput(Player player, String searchtext)
	{
		...
		{
			player.sendMessage(ChatApiOld.tl(plugin.getYamlHandler().getLang().getString("Log.LoggerSettingsTextOnlyThroughGUI")));
			return;
		}
	...
	public ItemStack generateItem(int slot, LoggerSettings fst, UUID uuid, boolean withEnd, boolean isPreset)
	{
		...
		im.setDisplayName(ChatApiOld.tl(plugin.getYamlHandler().getFilSet().getString(slot+".Name")));
		...
		if(isPreset)
		{
			...
			if(plugin.getMysqlHandler().exist(MysqlType.LOGGERSETTINGSPRESET, "`slotid` = ? AND `player_uuid` = ?", slotid, uuid.toString()))
			{
				ls = (LoggerSettings) plugin.getMysqlHandler().getData(MysqlType.LOGGERSETTINGSPRESET,
						"`slotid` = ? AND `player_uuid` = ?", slotid, uuid.toString());
				...
				for(int i = 0; i < actualValues.size(); i++)
				{
					...
					if(fst.getActionFilter() != null)
					{
						...
						{
							s = s.replace("%category%", ChatApiOld.tl(ls.getActionFilter().getCategory()));
							lore.add(s);
							continue;
						}
						if(s.contains("%comment%") && ls.getActionFilter().getComment() != null)
						{
							s = s.replace("%comment%", ChatApiOld.tl(ls.getActionFilter().getComment()));
							lore.add(s);
							continue;
						}
					}
							...
							&& (ls.getMin() != null || ls.getMax() != null))
					{
						if(ls.getMin() != null)
						{
							s = s.replace("%min%", AEP.getVault().format(ls.getMin()));
						} else
						{
							s = s.replace("%min%", "/");
						}
						if(ls.getMax() != null)
						{
							s = s.replace("%max%", AEP.getVault().format(ls.getMax()));
						} else
						{
							s = s.replace("%max%", "/");
						}
					...
					{
						s = s.replace("%firststand%", AEP.getVault().format(ls.getTrendfFilter().getFirstStand()));
						lore.add(s);
						continue;
					}
					if(s.contains("%laststand%") && ls.getTrendfFilter().getLastStand() != null)
					{
						s = s.replace("%laststand%", AEP.getVault().format(ls.getTrendfFilter().getLastStand()));
						lore.add(s);
						continue;
					}
				}
		...
		if(withEnd)
		{
			...
			for(int i = 0; i < actualValues.size(); i++)
			{
				...
				if(fst.getActionFilter() != null)
				{
					...
					{
						s = s.replace("%category%", ChatApiOld.tl(fst.getActionFilter().getCategory()));
						lore.add(s);
						continue;
					}
					if(s.contains("%comment%") && fst.getActionFilter().getComment() != null)
					{
						s = s.replace("%comment%", ChatApiOld.tl(fst.getActionFilter().getComment()));
						lore.add(s);
						continue;
					}
				}
						...
						&& (fst.getMin() != null || fst.getMax() != null))
				{
					if(fst.getMin() != null)
					{
						s = s.replace("%min%", AEP.getVault().format(fst.getMin()));
					} else
					{
						s = s.replace("%min%", "/");
					}
					if(fst.getMax() != null)
					{
						s = s.replace("%max%", AEP.getVault().format(fst.getMax()));
					} else
					{
						s = s.replace("%max%", "/");
					}
				...
				{
					s = s.replace("%firststand%", AEP.getVault().format(fst.getTrendfFilter().getFirstStand()));
					lore.add(s);
					continue;
				}
				if(s.contains("%laststand%") && fst.getTrendfFilter().getLastStand() != null)
				{
					s = s.replace("%laststand%", AEP.getVault().format(fst.getTrendfFilter().getLastStand()));
					lore.add(s);
					continue;
				}
			}
	...
	public void forwardingToOutput(Player player, LoggerSettings fst, LoggerSettingsHandler.Access access, Methode methode, int page, String cmdString) throws IOException
	{
		new BukkitRunnable()
		{
			...
			public void run()
			{
				...
				if(ac == null)
				{
					player.sendMessage(ChatApiOld.tl(plugin.getYamlHandler().getLang().getString("Log.AccountDontExit")));
					new BukkitRunnable()
					{
						@Override
						public void run()
				...
				if(Methode.BARCHART == methode)
				{
					...
					int last = plugin.getMysqlHandler().getCount(MysqlType.ACTION, query, whereObject);
				...
				} else if(Methode.DIAGRAM == methode)
				{
					...
					} else
					{
						...
						ArrayList<TrendLogger> list = ConvertHandler.convertListIV(
								plugin.getMysqlHandler().getList(MysqlType.TREND, order, start, end,
										query, whereObject));
