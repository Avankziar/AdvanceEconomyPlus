...
import me.avankziar.ifh.spigot.economy.currency.CurrencyGradation;
...
public class CurrencyHandler
{
	...
	{
		ConfigHandler.debug(d1, "> registerCurrencyFromFile start : "+c.getNameAsString());
	...
	{
		return plugin.getMysqlHandler().getCount(MysqlType.ACCOUNT_SPIGOT, "`id`", "`account_currency` = ?", currency.getUniqueName());
	}

	public double getTotalMoneyInTheSystem(EconomyCurrency currency, AccountType type)
	{
		return plugin.getMysqlHandler().getCount(MysqlType.ACCOUNT_SPIGOT, "`id`",
