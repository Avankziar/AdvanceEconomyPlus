...
public class Transfer extends ArgumentModule implements CommandExecutor
{
	...
	{
		ArrayList<AccountManagement> manaI = ConvertHandler.convertListIX(plugin.getMysqlHandler().getFullList(
				MysqlType.ACCOUNTMANAGEMENT, "`id` ASC", "`account_id` = ? AND `account_management_type` = ?",
				from.getID(), AccountManagementType.CAN_RECEIVES_NOTIFICATIONS.toString()));
		ArrayList<AccountManagement> manaII = ConvertHandler.convertListIX(plugin.getMysqlHandler().getFullList(
				MysqlType.ACCOUNTMANAGEMENT, "`id` ASC", "`account_id` = ? AND `account_management_type` = ?",
		...
		{
			AEPUser u = (AEPUser) plugin.getMysqlHandler().getData(MysqlType.PLAYERDATA, "`player_uuid` = ?", acm.getUUID().toString());
			if(u == null)
			{
				continue;
			}
	...
	public static void sendToOther(AEP plugin, Account to, ArrayList<String> list, UUID...exceptions)
	{
		ArrayList<AccountManagement> mana = ConvertHandler.convertListIX(plugin.getMysqlHandler().getFullList(
				MysqlType.ACCOUNTMANAGEMENT, "`id` ASC", "`account_id` = ? AND `account_management_type` = ?",
				to.getID(), AccountManagementType.CAN_RECEIVES_NOTIFICATIONS.toString()));
		if(mana.isEmpty())
		{
