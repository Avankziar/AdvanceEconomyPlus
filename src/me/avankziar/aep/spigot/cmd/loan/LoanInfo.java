...
public class LoanInfo extends ArgumentModule
{
	...
	public void run(CommandSender sender, String[] args) throws IOException
	{
		...
		if(args.length >= 2)
		{
			...
			lr = (LoanRepayment) plugin.getMysqlHandler().getData(MysqlType.LOAN, "`id` = ?", id);
		...
		m1.add(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("Cmd.Loan.Info.Headline")
				.replace("%id%", ids)
				.replace("%name%", lr.getName())));
		msg.add(m1);
		ArrayList<BaseComponent> m2 = new ArrayList<>();
		m2.add(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("Cmd.Loan.Info.Participants")
				.replace("%debtor%", ldb)
				.replace("%owner%", low)));
		msg.add(m2);
		ArrayList<BaseComponent> m8 = new ArrayList<>();
		m8.add(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("Cmd.Loan.Info.Accounts")
		...
		m3.add(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("Cmd.Loan.Info.Amounts")
		...
		m4.add(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("Cmd.Loan.Info.Interest")
		...
		m9.add(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("Cmd.Loan.Info.Tax")
		...
		m5.add(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("Cmd.Loan.Info.Ratio")
		...
		m6.add(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("Cmd.Loan.Info.Times")
		...
		m7.add(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("Cmd.Loan.Info.TheoreticalNumberOfRates")
