...
public class LoanSend extends ArgumentModule
{
	...
	public void run(CommandSender sender, String[] args)
	{
		...
		player.spigot().sendMessage(
				ChatApiOld.hover(plugin.getYamlHandler().getLang().getString("Cmd.Loan.Send.YourProposal")
						.replace("%player%", args[2]),
				HoverEvent.Action.SHOW_TEXT, hover));
		TextComponent tx = ChatApiOld.hover(
				plugin.getYamlHandler().getLang().getString("Cmd.Loan.Send.AProposal")
				.replace("%player%", player.getName()),
				HoverEvent.Action.SHOW_TEXT, hover);
		ArrayList<BaseComponent> list = new ArrayList<>();
		list.add(tx);
		TextComponent ar = ChatApiOld.generateTextComponent(plugin.getYamlHandler().getLang().getString("Cmd.Loan.Send.AcceptReject")
