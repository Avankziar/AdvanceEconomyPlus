...
public class AccountHandler
{
		...
		for(EconomyCurrency ec : plugin.getIFHApi().getCurrencies(CurrencyType.DIGITAL))
		{
			...
			if(y.getBoolean("WhenPlayerFirstJoin.CreateWallets", false))
			{
				...
				for(String a : y.getStringList("WhenPlayerFirstJoin.WalletsToCreate"))
				{
					...
					AccountBungee ac = new AccountBungee(acname, at, acy, ec, ee, startamount, defaultac);
					if(plugin.getIFHApi().existAccount(ee.getUUID(), ec, ac.getType(), acy, eeet))
					{
						continue;
					}
					plugin.getMysqlHandler().create(MysqlType.ACCOUNT_BUNGEE, ac);
					for(AccountManagementType amt : amtl)
					{
						if(!plugin.getIFHApi().canManageAccount((Account) ac, ee.getUUID(), amt))
						{
							plugin.getIFHApi().addManagementTypeToAccount((Account) ac, ee.getUUID(), amt);
						}
					}
				}
			}
	...
	public ArrayList<EconomyEntity> getEntitys(EconomyEntity.EconomyType type)
	{
		try
		{
			...
			case PLAYER:
				return ConvertHandler.convertList0(plugin.getMysqlHandler().getFullList(MysqlType.ACCOUNT_BUNGEE, "`id` ASC", "1"));
			}
		} catch(Exception e)
		{
	...
	{
		return plugin.getMysqlHandler().exist(MysqlType.ACCOUNT_BUNGEE, 
	...
	public boolean createAccount(Account account)
	{
		...
		return plugin.getMysqlHandler().create(MysqlType.ACCOUNT_BUNGEE, new AccountBungee(account));
	}
			...
			EconomyEntity owner, double balance)
	{
		...
		return plugin.getMysqlHandler().create(MysqlType.ACCOUNT_BUNGEE, 
				new AccountBungee(accountName, type, category, currency, owner, balance, false));
	}
	
	public int deleteAccount(Account account)
	{
		return plugin.getMysqlHandler().deleteData(MysqlType.ACCOUNT_BUNGEE,
				"`owner_uuid` = ? AND `account_currency` = ? AND `account_type` = ? AND `account_category` = ? AND `owner_type` = ?",
	...
	{
		return (Account) plugin.getMysqlHandler().getData(MysqlType.ACCOUNT_BUNGEE, "`id` = ?", id);
	}
	...
	public Account getAccount(UUID ownerUUID, String accountName)
	{
		...
		return (Account) plugin.getMysqlHandler().getData(MysqlType.ACCOUNT_BUNGEE,
				  "`owner_uuid` = ? AND `account_name` = ?",
	...
	{
		return ConvertHandler.convertListII(plugin.getMysqlHandler().getFullList(MysqlType.ACCOUNT_BUNGEE,
				"`id` ASC", "1"));
	}
