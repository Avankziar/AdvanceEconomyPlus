...
public class TrendLog implements CommandExecutor
{
	...
	public TrendLog(AdvancedEconomyPlus plugin, CommandConstructor cc, ArgumentConstructor ac, CommandStructurType cst)
	{
		this.plugin = plugin;
		this.cc = cc;
		this.ac = ac;
			...
			int zero, int one, int two) throws IOException
	{
		...
		new LoggerSettingsHandler(plugin).forwardingToOutput(player, fst, LoggerSettingsHandler.Access.COMMAND, Methode.LOG, page, CommandSuggest.TRENDLOG);
		return;
	}
}