	...
	public void run(CommandSender sender, String[] args)
	{
		...
		if(!AEPSettings.settings.isPlayerAccount())
		{
			player.sendMessage(ChatApi.tl(
					plugin.getYamlHandler().getLang().getString("NoBank")));
			return;
		}
		...
		if(eco.isMoneyPlayerFlow())
		{
			...
			player.sendMessage(ChatApi.tl(plugin.getYamlHandler().getLang().getString("CmdMoney.Toggle.SetOff")));
		} else
		{
			eco.setMoneyPlayerFlow(true);
			plugin.getMysqlHandler().updateData(Type.PLAYER, eco, "`id` = ?", eco.getId());
			player.sendMessage(ChatApi.tl(plugin.getYamlHandler().getLang().getString("CmdMoney.Toggle.SetOn")));
		}
		return;
	}
}