...
public class ActionLog implements CommandExecutor
{
	...
	public ActionLog(AdvancedEconomyPlus plugin, CommandConstructor cc, ArgumentConstructor ac, CommandStructurType cst)
	{
		this.plugin = plugin;
		this.cc = cc;
		this.ac = ac;
			...
			int zero, int one, int two) throws IOException
	{
		...
		if(args.length == zero+1)
		{
			...
			accountID = fromuser.getShortPayAccountID();
		...
		new LoggerSettingsHandler(plugin).forwardingToOutput(player, fst, LoggerSettingsHandler.Access.COMMAND, Methode.LOG, page, CommandSuggest.ACTIONLOG);
		return;
	}
}