...
public class BankAccountHandler
{
	...
	public static BankAccount createBankAccount(AdvancedEconomyPlus plugin, String accountNumber, String name, String ownerUUID)
	{
	...
	{
		BankAccount ba = (BankAccount) AdvancedEconomyPlus.getPlugin().getMysqlHandler().getData(MysqlHandler.Type.BANKACCOUNT, "id", id);
		return ba;
	}
	
	public static BankAccount getBankAccount(String accountNumber)
	{
		BankAccount ba = (BankAccount) AdvancedEconomyPlus.getPlugin().getMysqlHandler().getData(MysqlHandler.Type.BANKACCOUNT, "accountnumber", accountNumber);
		return ba;
	}
	...
	{
		int end = AdvancedEconomyPlus.getPlugin().getMysqlHandler().lastID(MysqlHandler.Type.BANKACCOUNT);
		if(AdvancedEconomyPlus.getPlugin().getMysqlHandler().getList(MysqlHandler.Type.BANKACCOUNT,
	...
	private static String TIMES_ONE(long charamount)
	{
		...
		while(first<charamount)
		{
			...
			{
				BankAccount ba = getBankAccount(n);
				if(ba == null)
				{
					check = true;
					accountNumber = n;
